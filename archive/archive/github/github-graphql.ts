...
import { GitHubRequestorError } from '../../src/models.js'; // Import models if needed, currently unused

// Define RepoSearchSort and RepoSearchOrder types
export type RepoSearchSort =
  | 'stars'
  | 'forks'
  | 'help-wanted-issues'
  | 'updated';
export type RepoSearchOrder = 'asc' | 'desc';
...
export default class GitHubGraphQL {
  ...
  async searchIssuesAndPrs(
    ...
    this.requestDelayMs = this.requestDelayMs ?? 500;

    while (fetched < perPage && hasNextPage && currentPage <= page) {
      // Wait before each request to avoid hitting rate limits
      if (currentPage > 1) await this.wait(this.requestDelayMs);
      const variables = {
        query,
        first: Math.min(perPage, 100),
        after,
      };
  ...
  private requestDelayMs: number = 500;
